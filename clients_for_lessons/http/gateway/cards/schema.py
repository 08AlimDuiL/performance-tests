...
'''
Все модели вынесены в /сlients/http/gateway/cards/schema.py, теперь клиент CardsGatewayHTTPClient просто импортируют их.
CardSchema переиспользуется в IssueVirtualCardResponseSchema, IssuePhysicalCardResponseSchema, что уменьшает количество повторяющегося кода.
'''
...
class IssueVirtualCardRequestSchema(BaseModel):
    ...
    user_id: str = Field(alias="userId") # помогают соответствовать JSON-структуре API, в котором используется camelCase
    account_id: str = Field(alias="accountId")
...
class IssuePhysicalCardRequestSchema(BaseModel):
    ...
    """
    model_config = ConfigDict(populate_by_name=True)  #Эта настройка позволяет обращаться к полям по их Python-именам (snake_case) при создании экземпляров моделей.
